...
function Square({ index, onClick, player }: Props) {
  ...
  const textColor = player === "X" ? "text-yellow-300" : "text-purple-400";
  const hoverStyle = "transition duration-500 hover:scale-105 transform";
  return (
    <div
      data-cell-index={index}
      className={`h-36 border-solid border-4 border-slate-200 font-Fredoka-One text-7xl text-center flex justify-center items-center cursor-pointer ${hoverStyle}`}
      {...{ onClick }}
    >
      <span
        data-cell-index={index}
